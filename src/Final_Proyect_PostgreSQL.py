from tkinter import Tk, Canvas, Frame, Label, Entry, Button, W, E, Listbox, END, ttk, CENTER
...
def Nuevo_Calculo(KgPimientos, PHumedad, sreq, dias, Hdias, EfeColector,
    ...
    conn = psycopg2.connect(dbname="SolarDryers", user="postgres",
                            password="1", host="localhost", port="5432")
    cursor = conn.cursor()
    query = '''INSERT INTO SolarDryersData(nd, np) VALUES (%s, %s)'''
    cursor.execute(query, (nd, np))
    conn.commit()
    conn.close()
    # refresh database
...
def display_calculos(frame):
    conn = psycopg2.connect(dbname="SolarDryers", user="postgres",
                            password="1", host="localhost", port="5432")
    cursor = conn.cursor()
    query = '''SELECT * FROM SolarDryersData'''
